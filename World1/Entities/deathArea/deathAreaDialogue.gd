...
var letters = ["He's gone... What could've happened to him?"]
...
func finish(): #cleanup function for exiting dialogue
	...
	get_tree().get_root().get_node("Node/YSort/Player").movableDialogue = false
	get_tree().get_root().get_node("Node/YSort/Player").justExited = true
